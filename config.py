...
def _saved_user_count() -> int:
    ...
    try:
        ...
        if isinstance(plugin_section, dict) and "user_count" in plugin_section:
            return max(1, int(plugin_section.get("user_count", 1)))

        owner_auth = data.get("owner_auth") if isinstance(data, dict) else None
        if isinstance(owner_auth, dict):
            if "User" in owner_auth:
                return max(1, int(owner_auth.get("User", 1)))
            users = owner_auth.get("users")
            if isinstance(users, list):
                configured = sum(1 for user in users if isinstance(user, dict) and user.get("owner_qq"))
                if configured:
                    return configured
        ...
        legacy_slots = [
            int(key[4:])
            for key in data
            if isinstance(key, str) and key.startswith("user") and key[4:].isdigit()
        ]
        if legacy_slots:
            return max(legacy_slots)
        return 1
