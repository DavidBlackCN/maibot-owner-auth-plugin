...
except ImportError:  # pragma: no cover - fallback for loaders that import plugin.py as a loose module.
    from config import (
        DEFAULT_NON_OWNER_TEMPLATE as _DEFAULT_NON_OWNER_TEMPLATE,
        DEFAULT_USER_TEMPLATE as _DEFAULT_USER_TEMPLATE,
        PLUGIN_VERSION,
...
class OwnerAuthPlugin(MaiBotPlugin):
    ...
    def _migrate_legacy(self, data: dict[str, Any], disk: dict[str, Any]) -> bool:
        ...
        - 本 fork 2.0.x 的 [owner_auth] + [[owner_auth.users]] 列表结构。
        ...
        if isinstance(owner_auth, dict):
            ...
            # This fork's 2.0.x SDK branch used [[owner_auth.users]] with
            # nickname / owner_qq / prompt_template. Migrate those entries into
            # the new fixed user slots so upgrades keep existing identities.
            owner_users = owner_auth.get("users")
            if isinstance(owner_users, list):
                entries = [
                    entry
                    for entry in owner_users
                    if isinstance(entry, dict) and self._norm_qq(entry.get("owner_qq"))
                ]
                if entries:
                    plugin_sec = data.setdefault("plugin", {})
                    desired_count = min(max(1, len(entries)), _USER_SLOTS)
                    if plugin_sec.get("user_count") != desired_count:
                        plugin_sec["user_count"] = desired_count
                        changed = True
                for idx, entry in enumerate(entries[:_USER_SLOTS], start=1):
                    target = data.setdefault(f"user{idx}", {})
                    vals = {
                        "enabled": True,
                        "qq": self._norm_qq(entry.get("owner_qq")),
                        "nickname": entry.get("nickname"),
    ...
    def _message_text(message: dict[str, Any]) -> str:
        ...
        if not isinstance(text, str):
            ...
            if isinstance(info, dict):
                text = info.get("processed_plain_text") or info.get("raw_message")
        if not isinstance(text, str):
            return ""
        ...
        try:
            ...
            self._recent_by_session[session_id] = {
                ...
                "processed_plain_text": (
                    ...
                    or (info.get("processed_plain_text") if isinstance(info, dict) else "")
                    or (info.get("raw_message") if isinstance(info, dict) else "")
                ),
